...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-lg border px-4 py-3 text-sm font-semibold ${status === 'success' ? 'border-[var(--slot4-accent)]/30 bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]' : 'border-red-500/30 bg-red-500/10 text-red-300'}`}>{message}</p> : null}
